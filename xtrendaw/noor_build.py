...
# علامات بداية المتن — بنقصّ من **آخر** علامة (بداية المتن عادة).
# مجموعة ١: فيها ذكر النبي ﷺ (أولوية). مجموعة ٢: علامات سرد عامة.
_PROPHET_MARKERS = [
    "قال رسول الله", "قال النبي", "عن النبي", "عن رسول الله",
    "أن رسول الله", "أن النبي", "أنه قال رسول الله", "عن أبي هريرة",
    "رسول الله صلى الله عليه وسلم قال", "النبي صلى الله عليه وسلم قال",
]
_GENERIC_MARKERS = ["قال:", "يقول:", "فقال:", "أنه قال:"]
# كلمات وصل لازم تدخل مع البداية عشان الجملة تبقى كاملة (تحسين 2026-09-25)
_CONNECTORS = {"أن", "أنه", "أنها", "عن", "عنِ", "قال", "فقال", "يقول",
               "قالت", "رضي", "رضى"}
# إشارات إن فيه سلسلة رواة (إسناد) في أول النص
_ISNAD_HINTS = ["حدثنا", "حدثني", "أخبرنا", "أخبرني", "عن مالك", "رضى الله عنه",
                "عنه قال"]


def _clean_display(text: str) -> str:
    """تنظيف **شكلي** للنص المعروض على الشاشة (من غير أي تغيير في كلمات الحديث).

    بيتشال: علامات العُرف الزخرفية («"» ‏ ‎ ﴿﴾)، والمسافات المتعددة.
    التشكيل والنص الأصلي بيفضلوا زي ما هم — الأمانة في نقل الحديث أولوية.
    """
    t = (text or "")
    for ch in ("\u200f", "\u200e", "\u00ab", "\u00bb", '"', "'", "\u201c",
               "\u201d", "\u2018", "\u2019"):
...
def _matn(text: str) -> str:
    ...
    الإسناد («حدثنا عبد الله بن مسلمة عن مالك عن سمي عن أبي صالح عن أبي
    هريرة…») مش مقصود للمشاهد — بيضيّع ٨–١٠ ثواني من الفيديو ويشوّش، والخبر
    كله في المتن. بنقصّ من **آخر** ذكر للنبي ﷺ (بداية المتن عادةً).

    اتصلحت 2026-09-25: الصيغة الشائعة «أَنَّ رَسُولَ اللَّهِ ﷺ قَالَ» ما كانتش
    في قائمة العلامات، فالإسناد كان بيتنطق كامل في كل حلقة (بق حقيقي).
    """
    t = _clean_display(re.sub(r"\s+", " ", (text or "").strip()))
    bare, idx = _strip(t)
    # الأولوية لعلامة فيها ذكر النبي ﷺ (أوضح وأكرم في السرد)، وبعدها العامة.
    prophet = max((bare.rfind(m) for m in _PROPHET_MARKERS), default=-1)
    generic = max((bare.rfind(m) for m in _GENERIC_MARKERS), default=-1)
    best = prophet if prophet > 0 else generic
    if best <= 0 and any(h in bare for h in _ISNAD_HINTS):
        # مفيش علامة صريحة + فيه إسناد → آخر «رسول الله» / «النبي» في النص
        best = max(0, bare.rfind("رسول الله"), bare.rfind("النبي"))
    if best <= 0:
        return t
    # نوسّع البداية لكلمة الوصل اللي قبلها (أن / عن) — بدونها الجملة تبدأ ناقصة
    for _ in range(2):
        j1 = bare.rfind(" ", 0, best)          # المسافة اللي قبل العلامة
        if j1 < 0:
            break
        j2 = bare.rfind(" ", 0, j1)            # المسافة اللي قبل الكلمة
        word = bare[j2 + 1:j1].strip()
        if word in _CONNECTORS:
            best = j2 + 1
        else:
            break
    cut = t[idx[best]:]
    # لو القصّ ساب أقل من 25 حرف (قصّ غلط) نرجع الأصل بدل متن ناقص.
    return cut if len(cut) > 25 else t
