...
# ───────── تطبيع النطق العربي — نسخة 2026-09-25 (نطق سليم 100%) ─────────
# ليه ده مهم: كل خطأ في النطق = فيديو «مش محترف» وبيقلل وقت المشاهدة.
# اللي بنصلحه هنا بالترتيب:
#   1) الرموز اللي بيتقرا مكانها كلام غلط أو بتوقّف الصوت (ﷺ · ﷻ · ﴿ ﴾ · إيموجي).
#   2) علامات الوقف القرآني (ۖ ۗ ۘ ۚ ۛ ۞ ۩) — تتشال، بتخبّط النطق.
#   3) الألف الخنجرية (U+0670): «الرَّحۡمَٰن» المفروض تُقرأ «الرَّحمان» — نحوّلها «ا».
#   4) الأرقام العربية-الهندية ٠١٢٣ → 0123 — الصوت العربي يقراها عربي صح.
#   5) الحروف المقطّعة (الم · يس · طه · ص · ق · ن · حم · كهيعص) — كانت بتتقرا
#      «ا ل م» أو غلط؛ بتتوسّع لكلماتها المنطوقة الصح.
#   6) حروف فارسية/بديلة (ک ی ھ) و التطويل (ـ) — بتوحّد للعربي الفصيح.
_SPEECH_MAP = [
    ("ﷺ", " صلى الله عليه وسلم "), ("ﷻ", " جل جلاله "),
    ("ﷲ", " الله "), ("﷽", " بسم الله الرحمن الرحيم "),
    ("ﻻ", "لا"), ("ٱ", "ا"), ("ـ", ""),
    ("﴿", " "), ("﴾", " "),
    ("ک", "ك"), ("ی", "ي"), ("ې", "ي"), ("ھ", "ه"), ("ۀ", "ة"),
]
...
# علامات الوقف والرموز القرآنية (بتتشال لأنها مش منطوقة)
_MARKS = re.compile("[\u0610-\u061a\u06d6-\u06ed\u0640]")
# الحروف المقطّعة → نطقها الصح (فواتح السور)
_MUQATTAAT = {
    "الم": "ألف لام ميم", "المص": "ألف لام ميم صاد", "المر": "ألف لام ميم را",
    "الر": "ألف لام را", "كهيعص": "كاف ها يا عين صاد",
    "حمعسق": "حا ميم عين سين قاف", "عسق": "عين سين قاف", "طسم": "طا سين ميم",
    "طس": "طا سين", "طه": "طا ها", "يس": "يا سين", "حم": "حا ميم",
    "ص": "صاد", "ق": "قاف", "ن": "نون",
}
def _expand_muqatta(t: str) -> str:
    """يوسّع فواتح السور **كتوكن كامل** بس.

    ⚠️ درس حقيقي: المطابقة بالبحث الحر بقت تلمّ «الر» جوه «الرَّحۡمَٰنِ» →
    «ألف لام راَّحا ميمَانِ». الصح: نقارن الكلمة كاملة (بعد شيل التشكيل)
    ومنغيّرهاش إلا لو هي فعلًا فاتحة سورة لوحدها.
    """
    import unicodedata
    out = []
    for tok in t.split(" "):
        core = "".join(ch for ch in tok
                       if unicodedata.category(ch) != "Mn")
        out.append(_MUQATTAAT[core] if core in _MUQATTAAT and core else tok)
    return " ".join(out)


def _drop_symbols(t: str) -> str:
    """يشيل الإيموجي والرموز اللي بتخبّط النطق (من غير حروف ولا ترقيم)."""
    import unicodedata
    kill = {"So", "Sk", "Cf", "Co", "Cs"}
    return "".join(ch for ch in t
                   if unicodedata.category(ch) not in kill)


def normalize_for_speech(text: str) -> str:
    """تطبيع للنطق: كل كلمة وكل حرف يوصل صح — نص نظيف للصوت بس.

    ملاحظة: ده للنطق فقط؛ النص اللي بيتكتب على الشاشة أو في الوصف
    ما يتغيّرش (القرآن يتكتب زي ما هو بالتشكيل).
    """
    t = text or ""
    for a, b in _SPEECH_MAP:
        t = t.replace(a, b)
    t = _MARKS.sub("", t)
    t = t.replace("\u0670", "ا")          # الألف الخنجرية → ألف تنطق
    t = t.translate(_DIGITS)
    t = _drop_symbols(t)
    for a, b in _PUNCT_MAP:
        t = t.replace(a, b)
    t = _expand_muqatta(t)
    t = re.sub(r"\s+([،.:!؟])", r"\1", t)   # مسافة زيادة قبل الترقيم
